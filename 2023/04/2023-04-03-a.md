...
Redux: with the fixes made to allow writing to SD card, can write the file out locally rather than messing around with UARTs. Blinkenlight to show progress. Gives bitwise identical result to ARMv6 and arm64 implementations. 
...
def main():
    sd = sdcard.SDCard(
        SPI(2, mosi=Pin("SD_MOSI"), miso=Pin("SD_MISO"), sck=Pin("SD_SCK")),
        Pin("SD_CS"),
    )
    os.mount(sd, "/sd")
